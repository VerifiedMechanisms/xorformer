...
$$ w_a:=\lvert P_a\rvert+\lvert N_a\rvert, \qquad w:=\max_a w_a, $$
...
$$ \lvert g^{-1}(1)\rvert \leq \sum_{a=1}^{s}2^{v-w_a}. $$
...
$$ C_a(x)=0 \qquad\Longleftrightarrow\qquad \left(\prod_{i\in P_a}(1-x_i)\right) \left(\prod_{j\in N_a}x_j\right) =1. $$
